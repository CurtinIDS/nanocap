...
from nanocap.gui.settings import QT
...
'''may need this if the normal QWidget doesnt work CHECK ON WINDOWS:
class QVTKRenderWindowInteractor(QtOpenGL.QGLWidget):
or
...
class QVTKRenderWindowInteractor(QtGui.QWidget):
    ...
    def setSizeHint(self,w,h):
    	self.SH = (int(w),int(h))	
    	
    def sizeHint(self):
        return QtCore.QSize(self.SH[0],self.SH[1])
    ...
    def mouseReleaseEvent(self, ev):
        #print "MOUSE RELEASE",self._ActiveButton,QtCore.Qt.LeftButton
        ctrl, shift = self._GetCtrlShift(ev)
        self._Iren.SetEventInformationFlipY(ev.x(), ev.y(),
                                            ctrl, shift, chr(0), 0, None)
        
        if self._ActiveButton == QtCore.Qt.LeftButton:
            self.emit(QtCore.SIGNAL("LeftButtonReleaseOverride(QEvent)"),ev)
            self._Iren.LeftButtonReleaseEvent()
        elif self._ActiveButton == QtCore.Qt.RightButton:
            self.emit(QtCore.SIGNAL("RightButtonReleaseOverride(QEvent)"),ev)
            self._Iren.RightButtonReleaseEvent()
        elif self._ActiveButton == QtCore.Qt.MidButton:
            self.emit(QtCore.SIGNAL("MidButtonReleaseOverride(QEvent)"),ev)
    ...
    def mouseMoveEvent(self, ev):
        ...
        self.emit(QtCore.SIGNAL("MouseMoveOverride(QEvent)"),ev)
        #print "Mouse move"
