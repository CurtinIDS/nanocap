...
#        row.addWidgets((self.MinLoopCk,))
        ...
        self.NMinima.setValue(10)
        self.NMinima.setFixedWidth(50)
        self.NMaxMinima = SpinBox()
        self.NMaxMinima.setValue(100)
        self.NMaxMinima.setFixedWidth(50)
        self.connect(self.NMinima, QtCore.SIGNAL('valueChanged(int)'), self.setNMinima)
        self.connect(self.NMaxMinima, QtCore.SIGNAL('valueChanged(int)'), self.setNMaxMinima)
        row.addWidgets((lb,self.NMinima,QtGui.QLabel("N Max Minima"),self.NMaxMinima))
        sep = self.SubOptionsWindows["MIN"].addSeparator(dummy=True)
