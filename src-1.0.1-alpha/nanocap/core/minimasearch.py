...
#                                         numpy.sum(self.uniqueMinimaCarbon[i].energy),"\n")
    ...
    def search(self,pointSet):
        '''
        Uses globals.NMinima,globals.NMaxMinima
        '''    
        
        steptime = time.time()
        printh("Searching for minima",threading.current_thread(),"NMinima",self.config.opts["NMinima"],
               "NMaxMinima",self.config.opts["NMaxMinima"])
        #for i in range(0,globals.NMinima):
        ...
        while(self.NUnique < self.config.opts["NMinima"] and NTotal < self.config.opts["NMaxMinima"]):
